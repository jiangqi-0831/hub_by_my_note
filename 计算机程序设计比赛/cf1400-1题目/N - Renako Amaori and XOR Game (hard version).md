...
游戏按顺序从 1 到 n 进行，所以**最后一个有效操作**的玩家决定了第 $k$ 位的最终状态，从而决定了谁获胜。因此我们从后往前扫描，找到第一个满足 di​ 的第 $k$ 位为 1 的下标 $i$，若 $i$ 为奇数则 Ajisai 赢，若为偶数则 Mai 赢。

实现时，先求出 $X$ 的最高位掩码，再倒序遍历数组即可，总时间复杂度 $O(n)$，满足题目限制。

# 知识点XOR
1. $a \oplus a =0$
2. XOR具有等号交换律$a \oplus b=c \iff a = b\oplus c$
3. 提取二进制某一位（用按位与）
```
	if (x & (1<<k))
```
